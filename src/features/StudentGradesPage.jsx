...
export default function StudentGradesPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Bảng điểm"
        description="Kết quả học tập theo học kỳ."
      />
      ...
      <div className="stats-grid">
        <StatCard
          ...
          value={
            view.scores.length
              ? calculateWeightedAverage(view.scores, data.courseSections, data.subjects)
              : '—'
          }
        />
        <StatCard
          ...
          value={
            cumulativeScores.length
              ? calculateWeightedAverage(cumulativeScores, data.courseSections, data.subjects)
              : '—'
          }
        />
        <StatCard
          label="Môn đã đạt"
          value={cumulativeScores.filter((item) => isPassed(item.total)).length}
        />
        <StatCard
          label="Môn chưa đạt"
          value={cumulativeScores.filter((item) => !isPassed(item.total)).length}
